...
<script>
	...
	const constructNodes = (width, height) => {
		console.log(
			`🚀 ~ file: sketch03.svelte ~ line 119 ~ constructNodes ~ data.numNodes > hexes.length`,
			data.numNodes,
			hexes.length
		);
		if (data.numNodes > hexes.length) {
			...
			console.log(`🚀 ~ file: sketch03.svelte ~ line 126 ~ constructNodes ~ hexes`, hexes);
		} else {
	...
	const sketch = () => {
		...
		console.log(`🚀 ~ file: sketch03.svelte ~ line 191 ~ sketch ~ context`, context);
